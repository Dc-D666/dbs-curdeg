...
### 阶段 2（第 2 周）：频道 + 版块 + 成员（含加入审核）✅ 已完成

**任务清单**：
- [x] models：`community.py` / `board.py` / `role.py` / `member.py` / `join_request.py` + 迁移（roles 提前到本阶段建表，频道创建时自动初始化 owner/admin/normal 默认组）
- [x] `community_service.py` + `communities.py`：创建（自动成为 owner 成员）、列表（含我加入的/公开的）、详情、编辑、解散（仅 owner）
- [x] `board_service.py` + `boards.py`：CRUD、排序、隐藏/关闭
- [x] `member_service.py` + `members.py`：加入（自由/审核/邀请三种方式）、退出、成员列表（按身份组分页）
- [x] 加入审核：新建 `join_requests` 表（申请用户/频道/状态/审批人），owner 审批通过后自动建 members 记录
- [x] **图片上传** `uploads.py`（类型/大小校验、随机文件名、日期分目录、返回 URL）+ 用户头像 `POST /users/me/avatar`
- [x] **频道状态调整** `PUT /communities/{id}/status`（0正常/1关闭=owner，2违规封禁=系统管理员）
- [x] 前端：频道发现/创建页、频道详情页（版块 tab）、加入/退出按钮、owner 管理面板（头像/封面上传、状态调整）
- [x] pytest：频道创建权限、版块 CRUD、三种加入方式、上传、状态调整（阶段 2 共 15 用例）

**验收标准**：
1. ✅ 创建频道 → 自动成为成员并出现在"我的频道"；
2. ✅ 审核制频道：申请加入 → owner 审批 → 通过后才可发帖；
3. ✅ 非 owner 编辑频道/版块返回 403+1002 无权限；
4. ✅ 线上可完整演示建频道、加版块；
5. ✅ 图片上传经 nginx 直服（/uploads/ 200）；
6. ✅ 频道状态调整权限正确（owner 开关、系统管理员封禁）。
...
- **🎨 设计参考基准：TDesign（腾讯设计体系）** —— 开发前端时参考 https://tdesign.tencent.com/ 的资料：
  - 设计价值观：克制的企业级视觉、清晰的信息层级、一致的交互模式；
  - 重点参考：色彩系统（中性灰阶/品牌色用法）、字体排版规范（字号/行高/字重阶梯）、圆角与阴影规范、组件交互细则（表单/列表/弹窗/导航）；
  - 用法：**参考其设计语言与交互模式**（而非直接引入 TDesign 组件库），把规范融入本项目 `tokens.css` 与自研组件；
  - 与"去 AI 味"规范互为补充：TDesign 的克制感正是对抗 AI 默认模板的手段。
